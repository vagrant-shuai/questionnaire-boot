...
import com.xs.form.request.QueryFormItemRequest;
import com.xs.form.request.QueryFormRequest;
import com.xs.form.request.SortFormItemRequest;
import com.xs.form.service.*;
import com.xs.form.util.FormAuthUtils;
import com.xs.form.util.FormDataUtils;
import com.xs.form.util.ShortIdUtils;
import com.xs.form.util.SortUtils;
import com.xs.form.vo.FormFieldVO;
import com.xs.form.vo.OperateFormItemVO;
import com.xs.form.vo.RecycleFormVO;
import com.xs.form.vo.UserFormDetailVO;
...
public class UserFormController {
    ...
    public Result createForm(@RequestBody UserFormEntity form) {
    ...
    public Result createFormByTemplate(@RequestBody FormTemplateEntity request) {
        UserFormEntity userForm = formTemplateService.createFormByTemplate(request);
    ...
    public Result listForms(QueryFormRequest.List request) {
        List<UserFormEntity> entityList = formService.list(Wrappers.<UserFormEntity>lambdaQuery().eq(ObjectUtil.isNotNull(request.getStatus()), UserFormEntity::getStatus, request.getStatus()).orderByDesc(BaseEntity::getUpdateTime));
    ...
    public Result queryMyForms(QueryFormRequest.Page request) {
    ...
    public Result queryMyFormFolderTree() {
        List<UserFormEntity> list = formService.list(Wrappers.<UserFormEntity>lambdaQuery().eq(UserFormEntity::getFolder, 1));
    ...
    public Result updateForm(@RequestBody UserFormEntity form) {
        ValidatorUtils.validateEntity(form, AddGroup.class);
    ...
    public Result queryRecycleForms(QueryFormRequest.Page request) {
        Page page = formService.page(request.toMybatisPage(), Wrappers.<UserFormEntity>lambdaQuery().eq(UserFormEntity::getDeleted, 1).orderByDesc(BaseEntity::getUpdateTime));
    ...
    public Result deleteRecycleForm(@RequestBody UserFormEntity FormEntity) {
        boolean remove = formService.remove(Wrappers.<UserFormEntity>lambdaQuery().eq(UserFormEntity::getFormKey, FormEntity.getFormKey()));
        if (remove) {
